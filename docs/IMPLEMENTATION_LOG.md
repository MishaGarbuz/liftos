...
| 2026-06-02 | **KPI trend deltas** (visual batch 3): each dashboard KPI now shows a ▲/▼ trend (`.kpi-delta`, new `--danger` token). Sessions / Weekly Sets / Consistency compare week-over-week; Top Set shows improvement from the lift's first to latest logged week. Deltas hide when there's no prior week to compare. Also fixed the KPI label copy "Top E1RM" → "Top Set" (it shows actual best weight). `index.html`, `css/app.css`, `js/pages/dashboard.js`. |
